...
export default function ChatSession() {
  ...
  useEffect(() => {
    ...
    const isMe = lastMessage.senderRole === "teacher";
    const newMsg: ChatMessage = {
      id: lastMessage.id || Math.random().toString(),
      sender: isMe ? "teacher" : "student",
  ...
  const handleEndSession = async () => {
    ...
    try {
      await teacherApi.endSession(sessionId, {
        summary: "Chat doubt session resolved successfully.",
        keyNotes: "Advised student to review the core formulas."
      });
    } catch { /* ignore */ }
          ...
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={cn("flex flex-col w-full", isMe ? "items-end" : "items-start")}>
                ...
                {isMe && (
                  <span className="text-[10px] text-cyan-400 font-black tracking-wide mb-1.5 mr-1">
                    You
                  </span>
                )}
