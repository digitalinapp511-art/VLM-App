...
export default function CreateProfileShadcn() {
  ...
  const validate = () => {
    ...
    if (!dateOfBirth) {
      ...
      currentDobError = "Date of birth is required";
    } else {
      ...
      if (birthDate.getFullYear() < today.getFullYear() - 100) {
        newErrors.dateOfBirth = true;
        currentDobError = "Enter a valid year (max 100 years old)";
      } else if (age < 8) {
        newErrors.dateOfBirth = true;
        currentDobError = "Student must be at least 8 years old";
      } else if (birthDate > today) {
        newErrors.dateOfBirth = true;
        currentDobError = "Future dates are not allowed";
      }
    }
        ...
        <SectionCard title="Personal Details" className="z-30">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Date of Birth *">
              ...
              />
              {errors.dateOfBirth && <span className="text-[10px] text-red-500 font-bold mt-0.5 block">{dobError || "Date of birth is required"}</span>}
            </Field>
          </div>
